...
class Rkf45 {
  ...
  static readonly float DoubleEpsilon = FindDoubleEpsilon();
  const int MAXNFE = 3000;

  private readonly Action<float, float[], float[]> f; 
  private readonly int neqn;
  private readonly float[] f1, f2, f3, f4, f5;

  // These were C "static" variables in R8_RKF45:
  private float abserr_save = -1.0f;
  ...
  private int kop = -1;
  private int nfe = -1;
  private float relerr_save = -1.0f;
  private float remin = 1.0E-12f;
  // This was a parameter in R8_RKF45:
  private readonly float[] yp;

  public Rkf45(Action<float, float[], float[]> f, int neqn) {
    this.f = f;
    if ( neqn < 1 )
      throw new Exception("There must be at least one equation >= 1");
    this.neqn = neqn;
    this.f1 = new float[neqn]; 
    this.f2 = new float[neqn]; 
    this.f3 = new float[neqn]; 
    this.f4 = new float[neqn]; 
    this.f5 = new float[neqn];
    this.yp = new float[neqn];
  }

  /******************************************************************************/

  private void r8_fehl (float[] y, float t, float h, float[] yp, float[] s)
  ...
  */
  {
    ...
    f ( t + 3.0f * h / 8.0f, f5, f2 );

    ch = h / 2197.0f;
    for (int i = 0; i < neqn; i++ )
      f5[i] = y[i] + ch * ( 1932.0f * yp[i] + ( 7296.0f * f2[i] - 7200.0f * f1[i] ) );
    f ( t + 12.0f * h / 13.0f, f5, f3 );
    
    ch = h / 4104.0f;
    for (int i = 0; i < neqn; i++ )
      f5[i] = y[i] + ch * ( ( 8341.0f * yp[i] - 845.0f * f3[i] ) + 
              ( 29440.0f * f2[i] - 32832.0f * f1[i] ) );
    f ( t + h, f5, f4 );
    
    ch = h / 20520.0f;
    for (int i = 0; i < neqn; i++ )
      f1[i] = y[i] + ch * ( ( -6080.0f * yp[i] + 
  ...
  public int r8_rkf45(float[] y, ref float t, float tout, 
		      ref float relerr, float abserr, int flag )
  ...
  /*
	...
	void f ( float t, float y[], float yp[] )
      ...
      Input/output, float YP[NEQN], the derivative of the current
      ...
      Input/output, ref float T, the current value of the independent
      ...
      Input, ref float RELERR, float ABSERR, the relative and absolute error 
  ...
  */
	...
	else if ( kflag == 5 && abserr == 0.0f )
	  throw new Exception("R8_RKF45 - Fatal error!\nKFLAG = 5 and ABSERR = 0.0f.\n" );
  ...
  */
	...
	else if ( flag_return == 5 && 0.0f < abserr )
	{
	  flag_return = flag_save;
	  if ( kflag == 3 )
	  {
  ...
  */
  ...
  /*
    ...
    accuracy requests.
  */
    relerr_min = 2.0f * DoubleEpsilon + remin;
  /*
  ...
  */
	...
	if ( 0.0f < tol )
	{
	  toln = tol;
	  ypk = Math.Abs( yp[k] );
	  if ( tol < ypk * (float)Math.Pow( h, 5 ) )
	  {
	    h = (float)Math.Pow( ( tol / ypk ), 0.2f );
	  }
	}
      }
	...
	h = 0.0f;
      ...
      h = Math.Max ( h, 26.0f * eps * Math.Max ( Math.Abs( t ), Math.Abs( dt ) ) );
  ...
  */
    if ( 2.0f * Math.Abs( dt ) <= Math.Abs( h ) )
    {
      kop = kop + 1;
    }
  /*
  ...
  */
    if ( Math.Abs( dt ) <= 26.0f * eps * Math.Abs( t ) )
    {
      t = tout;
      for (int i = 0; i < neqn; i++ )
      {
  ...
  */
      ...
      if ( 2.0f * Math.Abs( h ) <= Math.Abs( dt ) )
      {
      }
      else
  /*
  ...
  */
	...
	*/
	...
	for (int k = 0; k < neqn; k++ )
	{
	  ...
	  ee = Math.Abs 
	  ( ( -2090.0f * yp[k] 
	    + ( 21970.0f * f3[k] - 15048.0f * f4[k] ) 
	    ) 
	  + ( 22528.0f * f2[k] - 27360.0f * f5[k] ) 
	  );
	...
	esttol = Math.Abs( h ) * eeoet * scale / 752400.0f;
  ...
  */
	...
	if ( esttol < 59049.0f )
	{
	  s = 0.9f / (float)Math.Pow( esttol, 0.2f );
	}
	else
	{
	  s = 0.1f;
  ...
  */
  ...
  public static float[][] RKF45_n(Action<float, float[],float[]> dV, 
				   Action<float,float[]> bj_ii,
				   int a, int b, float err, float[] Va) {
    int n = Va.Length;
...
class TestRkf45 {
  ...
  public static void Zero(float t, float[] yv, /* for output: */ float[] yvp) {
    Debug.Assert(1 == yv.Length && yv.Length == yvp.Length);
    yvp[0] = 0;
  }

  public static void Two(float t, float[] yv, /* for output: */ float[] yvp) {
    Debug.Assert(1 == yv.Length && yv.Length == yvp.Length);
    yvp[0] = 2;
  }

  public static void Logistic(float t, float[] yv, /* for output: */ float[] yvp) {
    Debug.Assert(1 == yv.Length && yv.Length == yvp.Length);
    yvp[0] = 10 * yv[0] * (1 - yv[0]);
  }

  public static void DeferredTemporaryLifeAnnuity(float t, float[] yv, 
						  /* for output: */ float[] yvp) {
    Debug.Assert(1 == yv.Length && yv.Length == yvp.Length);
  ...
  static float GM(float t) {
    return 0.0005f + (float)Math.Pow(10.0f, 5.728f - 10.0f + 0.038f*(30f + t));
  }
}
...
class CalculationSpecifications {
  ...
  static float GM(float t) {
    return 0.0005f + (float)Math.Pow(10.0f, 5.728f - 10f + 0.038f*(age + t));
  }
